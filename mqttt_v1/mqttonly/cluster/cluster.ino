...
/*— HiveMQ Cloud tokens —*/
const char* mqttUser = "test1";            // fixed literal
...
String mac;
const char* ctrlTopic = "esp/control";     // one shared topic for all ESPs

// Pulse a relay for 100 ms
...
// Handle incoming messages: payload = "<targetMac>:<relayNum>"
void mqttCallback(char* topic, byte* payload, unsigned int len) {
  String p;
  for (unsigned int i = 0; i < len; i++) p += (char)payload[i];

  int sep = p.indexOf(':');
  if (sep < 0) return;           // malformed
  String targetMac = p.substring(0, sep);
  String relayNum  = p.substring(sep + 1);

  // Only act if it’s addressed to me:
  if (targetMac != mac) return;

  Serial.printf("Command for me → relay %s\n", relayNum.c_str());
...
void connectMQTT() {
  while (!mqtt.connected()) {
    Serial.print("MQTT → ");
    String clientId = "esp8266-" + mac;  // unique per ESP
    secureClient.setInsecure();         // skip cert validation (dev only)
...
void setup() {
  ...
  // Derive my MAC (no colons, lowercase)
  mac = WiFi.macAddress();
  mac.replace(":", "");
  mac.toLowerCase();
